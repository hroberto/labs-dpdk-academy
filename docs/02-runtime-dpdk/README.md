...
**E "morrer" custa coisas diferentes conforme o que foi reservado.** A mesma
medição, com a única diferença sendo o modo de memória da EAL:
...
[`rte_eal_cleanup()`][apiealclean] custa **0,12 ms com `-l 0 --in-memory`** e
**0,65 ms com `-l 0 --no-huge`** — três e duas ordens de grandeza abaixo da
inicialização, respectivamente. O número não existe sem a configuração ao lado,
e a [§2](#2-o-custo-de-existir-quanto-a-eal-leva-para-nascer) mostra por quê.
Sendo tão barato nos dois casos, a pergunta é o que acontece quando ele não é
chamado.
...
> **Os comentários vão acima, e não ao lado.** Numa continuação de linha, a
> `\` precisa ser o **último** caractere: `\ # comentário` escapa o espaço, não
> a quebra, e o `bash` passa a tratar a linha seguinte como comando novo.
