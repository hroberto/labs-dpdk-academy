...
of magnitude less. The asymmetry is the first relevant fact: being born is expensive,
dying is cheap.

> **RETRACTION — 16/09/2026: the shutdown cost was out of date, and the two languages
> disagreed with each other.** The Portuguese body published **0,08 ms** and "three
> orders of magnitude"; this same document's English summary published **0.30 ms** and
> "two orders of magnitude". Neither reproduces.
>
> Re-checked with six runs at the L2 test's parameters (`-l 0 --no-huge`, no `-m`):
> **0.63 ms**, range 0.56 to 0.67. The ratio against initialisation is **195×**, that
> is **two** orders of magnitude — the English was closer on the order and wrong on the
> number, the Portuguese wrong on both.
>
> The 123 ms `rte_eal_init()`, that one **does reproduce**: 123.0 to 123.6 ms in the
> same runs. And the shutdown cost depends on what was reserved — with `-m 256` it
> rises to 1.2 to 1.4 ms. That is why the configuration goes declared alongside the
> number, which was missing before.
>
> **The section's conclusion does not change**, and it is the reason it exists:
> shutting down remains orders of magnitude cheaper than starting up. But "three
> orders" became a repeated phrase, and it was wrong by a factor of eight.
...
> **Two methodological corrections in this table.** The previous version published
> 120,0 ms for `-l 0 --in-memory` — a value that fell **outside** the 121,3–123,9 range
> of the table at the start of the section, for the *same* configuration, without
> anything in the text noticing. And it claimed that "using four lcores changes
> nothing" without publishing any measurement with four lcores: the table had three
> rows.
>
> This collection corrects both. The range appears on every row, `-l 0-3` was actually
> measured, and the median for `-l 0 --in-memory` (122.4, range 121.6–123.7)
> **contains** the 123.1 of the title table — the two collections agree, which is what
> the previous version could not say.
>
> <!-- retratado: 122,3 120,7 -->
>
> (The 120,0 is deliberately NOT in the marker: the same digits reappear, with a
> legitimate role, as the lower end of the 120,0–122,5 range on the `--no-pci` row.
> The verifier's search is textual and does not separate the two roles.)
...
[`rte_eal_cleanup()`][apiealclean] costs 0.63 ms — two orders of magnitude less than
initialisation, with `-l 0 --no-huge` and no `-m` (see the retraction in
[§2](#2-the-cost-of-existing-how-long-the-eal-takes-to-be-born): the previous number,
0,08 ms, did not reproduce). Being so cheap, the question is what happens when it is not
called.
...
> **Why the comments are above, and not beside.** The previous version of this block put
> `\` followed by spaces and `#` on the same line, and **it did not work if copied**:
> the backslash escapes the space, not the line break. `bash` executes
> `./feed-primario '--lcores' ' '` and treats the next line as a new command. A line
> continuation requires the `\` to be the **last** character on the line.
