...
# parecer suficiente.
# Os conectores em INGLES nao estavam aqui, e o material e bilingue. O defeito
# que deu origem a este verificador -- "100 dos 123 ms, 83%" -- tem um gemeo
# traduzido em docs/02-runtime-dpdk/README.en.md, e esse gemeo nunca foi
# conferido uma unica vez. Um portao que cobre metade do corpus imprime o
# mesmo verde de um que cobre o corpus inteiro.
CONTA = re.compile(
    r"(?<![A-Za-z0-9])(\d+(?:[.,]\d+)?)\s+(?:d(?:e|os|as)|out of|of the|of)\s+"
    r"(\d+(?:[.,]\d+)?)"
    r"[^.;:%]{0,45}?(\d+(?:[.,]\d+)?)\s*%",
    re.IGNORECASE,
)
...
def verificar(raiz="."):
    ...
    for doc in sorted(arquivos(raiz)):
        ...
        if CITA.search(texto):
            ...
            continue
        # A VARREDURA E SOBRE O TEXTO INTEIRO, NAO LINHA A LINHA. Ela era por
        # linha, e por isso uma conta correta escapava do portao so por cair
        # em cima de uma quebra de linha: "8,5 dos 67,2 ns do orcamento" numa
        # linha e "12,6%" na seguinte nao casavam com nada. O texto nao muda
        # de significado onde o editor resolveu quebrar, e o verificador nao
        # pode depender disso. Escapar em silencio e o pior modo de falhar:
        # o verde continua igual.
        #
        # O preco e um risco novo -- casar numeros de PARAGRAFOS diferentes --
        # e ele e barrado logo abaixo: match que atravessa linha em branco e
        # descartado.
        for m in CONTA.finditer(texto):
            if "\n\n" in m.group(0):
                continue
            try:
            ...
            if abs(real - pct) > TOLERANCIA_PP:
                ...
                print(f"      {' '.join(m.group(0).split())[:110]}")
                problemas += 1
...
def autoteste():
    """Um caso certo, um errado, uma isca de falso positivo e a quebra de linha."""
    import io as _io
    import tempfile
    ...
    # 5. Conta errada QUEBRADA EM DUAS LINHAS: tem de falhar igual.
    #    Era este o buraco: a varredura ia linha a linha, e a mesma conta
    #    escapava inteira so por o editor ter quebrado a frase no meio.
    rc, _ = rodar("# t\n\n100 dos 123 ms do custo total,\n83% do total.\n")
    if rc != 1:
        print("  AUTOTESTE FALHOU: conta errada escapou por quebra de linha")
        falhas += 1

    # 6. Isca do parágrafo: numeros de PARAGRAFOS diferentes nao formam conta.
    #    E o risco que a mudanca acima introduz, e ele fica barrado.
    rc, _ = rodar("# t\n\nSao 100 dos 123 ms medidos.\n\n83% do que vem depois.\n")
    if rc != 0:
        print("  AUTOTESTE FALHOU: numeros de paragrafos diferentes viraram conta")
        falhas += 1

    # 7. O mesmo defeito EM INGLES: o corpus e bilingue, o portao tambem.
    rc, saida = rodar("# t\n\n100 of the 123 ms, 83% of the cost, is not work.\n")
    if rc != 1:
        print("  AUTOTESTE FALHOU: conta errada em ingles passou"); falhas += 1

    # 8. Isca em ingles: numerador maior que o denominador continua fora.
    rc, _ = rodar("# t\n\nMP/MC costs 435% more, of 8 objects against 128.\n")
    if rc != 0:
        print("  AUTOTESTE FALHOU: isca em ingles acusada"); falhas += 1
