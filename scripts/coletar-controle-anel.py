...
UNIT = 'ns por objeto; custo amortizado'
FIELDS = ('sample', 'bulk', 'burst', 'producer', 'consumer', 'objects', 'ns_per_object')
...
def validated_rows(folder):
    ...
    repetitions, cpus = metadata['repeticoes'], metadata['cpus']
    if (type(repetitions) is not int or repetitions < 3 or
        not isinstance(cpus, list) or len(cpus) != 2 or
        any(type(cpu) is not int or cpu < 0 for cpu in cpus) or cpus[0] == cpus[1]):
        raise ValueError('Protocolo de repeticoes/CPUs invalido')
    if metadata['unidade'] != UNIT:
        raise ValueError('Unidade incompativel com o protocolo')
    sources = metadata['fontes_exatas']
    if set(sources) != {'packet.hpp', 'controle-anel.cpp', 'coletar-controle-anel.py'}:
        raise ValueError('Manifesto de fontes incompleto')
    for name, digest in sources.items():
        if hashlib.sha256((folder / 'fontes' / name).read_bytes()).hexdigest() != digest:
            raise ValueError('Fonte divergente do manifesto: ' + name)
    if metadata.get('schema_version', 1) not in (1, 2):
        raise ValueError('Versao de protocolo desconhecida')
    if metadata.get('schema_version') == 2:
        if metadata['ambiente_returncode'] != 0:
            raise ValueError('Captura do ambiente falhou')
        artifacts = metadata['artefatos_sha256']
        if set(artifacts) != {'amostras.csv', 'execucoes.json', 'ambiente.txt', 'build.txt'}:
            raise ValueError('Manifesto de artefatos incompleto')
        for name, digest in artifacts.items():
            if hashlib.sha256((folder / name).read_bytes()).hexdigest() != digest:
                raise ValueError('Artefato divergente do manifesto: ' + name)
    with (folder / 'amostras.csv').open() as handle:
        reader = csv.DictReader(handle)
        if reader.fieldnames != ['repetition', *FIELDS]:
            raise ValueError('Colunas do CSV incompativeis')
        rows = list(reader)
    expected = {(r, bulk, burst, cpus[0], c) for r in range(repetitions)
                for bulk in (0, 1) for burst in (1, 8, 32, 128) for c in cpus}
    by_key = {}
    for row in rows:
        if set(row) != {'repetition', *FIELDS} or any(v is None for v in row.values()):
            raise ValueError('Linha CSV incompleta')
        key = tuple(int(row[k]) for k in ('repetition', 'bulk', 'burst', 'producer', 'consumer'))
        if key not in expected or key in by_key:
            raise ValueError('Cenario/repeticao inesperado ou duplicado')
        if int(row['sample']) != 0 or int(row['objects']) != 200000:
            raise ValueError('Unidade experimental divergente')
        value = float(row['ns_per_object'])
        if not math.isfinite(value) or value <= 0:
            raise ValueError('Amostra nao publicavel')
        by_key[key] = row
    if set(by_key) != expected:
        raise ValueError('Coleta incompleta: faltam cenarios ou repeticoes')
    runs = json.loads((folder / 'execucoes.json').read_text())
    seen = set()
    for run in runs:
        key = (run['repetition'], *run['case'])
        if key not in expected or key in seen or run['returncode'] != 0:
            raise ValueError('Execucao ausente, duplicada ou malsucedida')
        command = run['command']
        if len(command) != 6 or command[1:] != [*map(str, run['case']), '1']:
            raise ValueError('Comando divergente do cenario')
        reader = csv.DictReader(io.StringIO(run['stdout']))
        if reader.fieldnames != list(FIELDS):
            raise ValueError('Saida da execucao sem esquema esperado')
        parsed = list(reader)
        if parsed != [{k: by_key[key][k] for k in FIELDS}]:
            raise ValueError('CSV diverge da saida registrada')
        seen.add(key)
    if seen != expected:
        raise ValueError('Registro de execucoes incompleto')
    return rows


def table(folder):
    rows = validated_rows(folder)
    groups = {}
    for row in rows:
        value = float(row['ns_per_object'])
        if not math.isfinite(value) or value <= 0:
    ...
    atomic_write(folder / 'tabela.md', '\n'.join(lines))


def publish(folder):
    status = folder / 'publicacao-status.json'
    try:
        table(folder)
    except (OSError, ValueError, KeyError, TypeError) as error:
        atomic_write(status, json.dumps({'estado': 'FAIL', 'motivo': str(error)}, ensure_ascii=False) + '\n')
        raise ValueError('Publicacao recusada: ' + str(error)) from error
    atomic_write(status, json.dumps({'estado': 'VALIDO', 'tabela_sha256':
        hashlib.sha256((folder / 'tabela.md').read_bytes()).hexdigest()}) + '\n')
...
def main():
    ...
    metadata = {'schema_version': 2, 'data_utc': datetime.now(timezone.utc).isoformat(),
        ...
        'seed': 20260914, 'cpus': cpus, 'unidade': UNIT,
    ...
    metadata['ambiente_returncode'] = environment.returncode
    (folder / 'metadados.json').write_text(json.dumps(metadata, ensure_ascii=False, indent=2) + '\n')
    runs, samples = [], []
    ...
    try:
        if environment.returncode:
            raise RuntimeError('Captura do ambiente falhou; sem publicacao')
        ...
        metadata['artefatos_sha256'] = {name: hashlib.sha256((folder / name).read_bytes()).hexdigest()
            for name in ('amostras.csv', 'execucoes.json', 'ambiente.txt', 'build.txt')}
        (folder / 'metadados.json').write_text(json.dumps(metadata, ensure_ascii=False, indent=2) + '\n')
        publish(folder)
